...
from robogym.robot.ur5.arm_interface import (
...
class MujocoObservation(RobotObservation):
    ...
    def __init__(
        ...
        # embed()
        self._tcp_xyz = sim.data.get_body_xpos(TCP_BODY_NAME).copy()
        self._tcp_vel = sim.data.get_body_xvelp(TCP_BODY_NAME).copy()
        quat = sim.data.get_body_xquat(TCP_BODY_NAME).copy()
...
class JointControlledArm(Arm):
    """
    Mujoco implementation of a joint-actuated UR 5 arm.
    """
    ...
    def get_name(self) -> str:
        ...
        EPSILON, etc. for hands, and UR5-ONE for UR16 arms."""
        return "mujoco-ur5"  # note that all mujoco arms will have the same name
