from robogym.envs.push.common.utils import geom_ids_of_body
...
class ArmSimulationInterface(SimulationInterface):
    ...
    def __init__(
        ...
        self.register_joint_group("robot", prefix=["ur5"])
        # self.register_joint_group(
        #     "gripper", prefix=["s_model"]
        # )
        ...
        mocap_id = self.mj_sim.model.body_name2id("ur5_mocap")
    ...
    def make_robot_xml(cls, xml, robot_control_params):
        ...
        if robot_control_params.is_joint_actuated():
            ...
            xml.remove_objects_by_name("ur5_mocap")
            ...
            xml.append(
                MujocoXML.parse(
                    f"robot/ur5/jointspec/calibrations/{joint_subdir}/ur5_ik_class.xml"
                )
            )
            xml.append(
                MujocoXML.parse(
                    f"robot/ur5/jointspec/calibrations/{joint_subdir}/joint_actuations.xml"
                )
            )
        else:
            # If not joint control mode or ik solver mode, use mocap defaults for joint parameters
            xml.append(MujocoXML.parse("robot/ur5/jointspec/ur5_mocap_class.xml"))

        # Add gripper actuators now (after joint actuators if required).
        # xml.append(MujocoXML.parse("robot/ur5/gripper_actuators.xml"))
